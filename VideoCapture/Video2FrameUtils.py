...
filepath = 'E:\Video\\The Grand Budapest Hotel (2014) [1080p x265 HEVC 10bit BluRay AAC 5.1] [Prof].mkv'
...
def video2frames(pathIn='',
    ...
    print("Frames Times: {}" + "%d:%02d:%02d" % (h, m, s))
    print('------Video------Information----')

    ##提取特定时间点图片
    if extract_time_points is not None:
        ...
        while success and count < len(extract_time_points):
            ...
            if success:
                ...
                print('111111111111Write a new frame: {}, {}th'.format(success, count + 1))
                cv2.imwrite(os.path.join(pathOut, "{}_{}_{}.jpg".format(imgname,output_prefix, count + 1)), image,
            ...
            if end_extract_time is not None:
                ...
                while success and count < N:
                    ...
                    if success:
                        ...
                        print('2222222222222Write a new frame: {}, {}/{}'.format(success, count + 1, n_frames))
                        cv2.imwrite(os.path.join(pathOut, "{}_{}_{}.jpg".format(output_prefix, imgname, count + 1)),
                                    image, [int(cv2.IMWRITE_JPEG_QUALITY), jpg_quality])  # save frame as JPEG file
                        count = count + 1
            else:
                success = True
                count = 0
                while success:
                    ...
                    if success:
                        if not isColor:
                            image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                        print('33333333Write a new frame: {}, {}/{}'.format(success, count + 1, n_frames))
                        cv2.imwrite(os.path.join(pathOut, "{}_{}_{}.jpg".format(output_prefix, imgname, count + 1)),
                                    image, [int(cv2.IMWRITE_JPEG_QUALITY), jpg_quality])  # save frame as JPEG file
                        count = count + 1
...
if __name__ == "__main__":
    pathIn = 'E:\ThunderCloud\\2022.3.3\\IPX-833.mp4'
    pathOut = 'E:\ThunderCloud\\2022.3.3\\IPX-833'

    video2frames(pathIn, pathOut, "ABW-204", extract_time_interval=60, extract_time_points=None, jpg_quality=100)
    '''
        pathIn：视频的路径，比如：F:\python_tutorials\test.mp4
        pathOut：设定提取的图片保存在哪个文件夹下，比如：F:\python_tutorials\frames1\。如果该文件夹不存在，函数将自动创建它
        extract_time_points：提取的时间点，单位为秒，为元组数据，比如，(2, 3, 5)表示只提取视频第2秒， 第3秒，第5秒图片
